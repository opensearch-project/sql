...
public class CalcitePPLJoinIT extends CalcitePPLIntegTestCase {
  ...
  public void testMultipleJoinsWithRelationSubquery() {
    JSONObject actual =
        executeQuery(
            String.format(
                "source = %s| where country = 'Canada' OR country = 'England' | inner join left=a, right=b ON a.name = b.name AND a.year = 2023 AND a.month = 4 AND b.year = 2023 AND b.month = 4 [ source = %s]| eval a_name = a.name | eval a_country = a.country | eval b_country = b.country| fields a_name, age, state, a_country, occupation, b_country, salary| left join left=a, right=bON a.a_name = b.name [ source = %s ] | eval aa_country = a.a_country| eval ab_country = a.b_country | eval bb_country = b.country| fields a_name, age, state, aa_country, occupation, ab_country, salary, bb_country, hobby, language| cross join left=a, right=b[ source = %s] | eval new_country = a.aa_country | eval new_salary = b.salary | stats avg(new_salary) as avg_salary by span(age, 5) as age_span, state | left semi join left=a, right=bON a.state = b.state[source = %s ]| eval new_avg_salary = floor(avg_salary)| fields state, age_span, new_avg_salary ",
                TEST_INDEX_STATE_COUNTRY,
                TEST_INDEX_OCCUPATION,
                TEST_INDEX_HOBBIES,
                TEST_INDEX_OCCUPATION,
  ...
  public void testLeftJoinWithRelationSubquery() {
    JSONObject actual =
        executeQuery(
            String.format(
                "source = %s| where country = 'USA' OR country = 'England'| left join left=a, right=b ON a.name = b.name[ source = %s| where salary > 0 | fields name, country, salary | sort salary| head 3]| stats avg(salary) by span(age, 10) as age_span, b.country",
                TEST_INDEX_STATE_COUNTRY, TEST_INDEX_OCCUPATION));
    verifySchema(
        actual,
        schema("b.country", "string"),
