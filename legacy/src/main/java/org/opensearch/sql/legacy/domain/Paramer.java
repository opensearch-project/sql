      ...
      if (expr instanceof SQLCharExpr) {
        ...
        switch (Util.expr2Object(sqlExpr.getLeft()).toString()) {
          ...
          case "fields":
            int index;
            for (String f :
                Strings.splitStringByCommaToArray(
                    Util.expr2Object(sqlExpr.getRight()).toString())) {
              index = f.lastIndexOf('^');
              if (-1 < index) {
                instance.fieldsBoosts.put(
                    f.substring(0, index), Float.parseFloat(f.substring(index + 1)));
              } else {
          ...
          case "operator":
            instance.operator =
                Operator.fromString(Util.expr2Object(sqlExpr.getRight()).toString());
            break;
