...
/**
 * DynamicPivotOperator is responsible for implementing the two-phase approach for the timechart
...
public class DynamicPivotOperator {
  ...
  public static RelNode executePivot(
    ...
    for (String distinctValue : distinctValues) {
      ...
      relBuilder.projectPlus(relBuilder.alias(caseExpr, "pivot_" + distinctValue));
      ...
      pivotAggCalls.add(
          relBuilder
              .aggregateCall(SqlStdOperatorTable.MAX, relBuilder.field("pivot_" + distinctValue))
              .as(distinctValue));
    }
  ...
  /**
   * Get the distinct values for the "by" field from the result of a query. This method would be
   * called during execution, not during planning.
   *
