...
- ``language-grammar``: centralized package containing all ANTLR grammar files (.g4) used across the project.
...
**Purpose**: The language-grammar package enables sharing of grammar files between the main SQL repository and the Spark repository, ensuring consistency and reducing duplication.

**Updating Grammar Files**: When grammar files are modified in their respective modules (``sql/``, ``ppl/``, ``legacy/``, ``async-query-core/``), they must be manually copied to the ``language-grammar/src/main/antlr4/`` directory. Once updated, the package automatically triggers CI to upload the new version to Maven Central for consumption by other projects.
