...
public class WhereParser {
  ...
  private boolean explainSpecialCondWithBothSidesAreLiterals(SQLBinaryOpExpr bExpr, Where where)
    ...
    if ((bExpr.getLeft() instanceof SQLNumericLiteralExpr || bExpr.getLeft() instanceof SQLCharExpr)
        && (bExpr.getRight() instanceof SQLNumericLiteralExpr
            || bExpr.getRight() instanceof SQLCharExpr)) {
      ...
      sqlMethodInvokeExpr.addParameter(
          new SQLCharExpr(
              Util.expr2Object(bExpr.getLeft(), "'")
                  + " "
                  + operator
                  + " "
  ...
  private boolean explainSpecialCondWithBothSidesAreProperty(SQLBinaryOpExpr bExpr, Where where)
    ...
    if ((bExpr.getLeft() instanceof SQLPropertyExpr || bExpr.getLeft() instanceof SQLIdentifierExpr)
        && (bExpr.getRight() instanceof SQLPropertyExpr
            || bExpr.getRight() instanceof SQLIdentifierExpr)
  ...
  private void explainCond(String opear, SQLExpr expr, Where where) throws SqlParseException {
    if (expr instanceof SQLBinaryOpExpr) {
      ...
      if (!methodAsOpear) {
        ...
        if (isNested) {
          ...
          SQLMethodInvokeExpr sqlMethodInvokeExpr =
              parseSQLBinaryOpExprWhoIsConditionInWhere(soExpr);
      ...
      if (SpatialParamsFactory.isAllowedMethod(methodName)) {
        ...
        Object spatialParamsObject =
            SpatialParamsFactory.generateSpatialParamsObject(methodName, methodParameters);
        ...
        Condition condition =
            new Condition(Where.CONN.valueOf(opear), null, null, "SCRIPT", scriptFilter, null);
        where.addWhere(condition);
      ...
      Select innerSelect =
          sqlParser.parseSelect((MySqlSelectQueryBlock) sqlIn.getSubQuery().getQuery());
  ...
  private MethodField parseSQLMethodInvokeExprWithFunctionInWhere(SQLMethodInvokeExpr soExpr)
    ...
    MethodField methodField =
        fieldMaker.makeMethodField(
            soExpr.getMethodName(),
  ...
  private MethodField parseSQLCastExprWithFunctionInWhere(SQLCastExpr soExpr)
      throws SqlParseException {
    ArrayList<SQLExpr> parameters = new ArrayList<>();
    parameters.add(soExpr.getExpr());
    return fieldMaker.makeMethodField(
        "CAST", parameters, null, null, query != null ? query.getFrom().getAlias() : null, false);
  }
  ...
  private SQLMethodInvokeExpr parseSQLBinaryOpExprWhoIsConditionInWhere(SQLBinaryOpExpr soExpr)
    ...
    if (soExpr.getLeft() instanceof SQLMethodInvokeExpr) {
      if (!SQLFunctions.isFunctionTranslatedToScript(
          ((SQLMethodInvokeExpr) soExpr.getLeft()).getMethodName())) {
        return null;
      }
    }
    ...
    if (soExpr.getRight() instanceof SQLMethodInvokeExpr) {
      if (!SQLFunctions.isFunctionTranslatedToScript(
          ((SQLMethodInvokeExpr) soExpr.getRight()).getMethodName())) {
        return null;
      }
    }

    MethodField leftMethod =
        new MethodField(
            null,
            Lists.newArrayList(new KVValue("", Util.expr2Object(soExpr.getLeft(), "'"))),
            null,
            null);
    MethodField rightMethod =
        new MethodField(
            null,
            Lists.newArrayList(new KVValue("", Util.expr2Object(soExpr.getRight(), "'"))),
            null,
            null);

    if (soExpr.getLeft() instanceof SQLIdentifierExpr
        || soExpr.getLeft() instanceof SQLPropertyExpr) {
      leftMethod =
          new MethodField(
              null,
              Lists.newArrayList(
    ...
    if (soExpr.getRight() instanceof SQLIdentifierExpr
        || soExpr.getRight() instanceof SQLPropertyExpr) {
      rightMethod =
          new MethodField(
              null,
              Lists.newArrayList(
    ...
    if (soExpr.getLeft() instanceof SQLMethodInvokeExpr) {
      leftMethod =
          parseSQLMethodInvokeExprWithFunctionInWhere((SQLMethodInvokeExpr) soExpr.getLeft());
    }
    if (soExpr.getRight() instanceof SQLMethodInvokeExpr) {
      rightMethod =
          parseSQLMethodInvokeExprWithFunctionInWhere((SQLMethodInvokeExpr) soExpr.getRight());
    }
    ...
    String v1Dec =
        leftMethod.getParams().size() == 2
            ? leftMethod.getParams().get(1).value.toString() + ";"
            : "";

    String v2 = rightMethod.getParams().get(0).value.toString();
    String v2Dec =
        rightMethod.getParams().size() == 2
            ? rightMethod.getParams().get(1).value.toString() + ";"
            : "";
  ...
  private Boolean bothSideAreNotFunction(SQLBinaryOpExpr soExpr) {
    return !(soExpr.getLeft() instanceof SQLMethodInvokeExpr
      ...
      if (innerExpr instanceof SQLQueryExpr) {
        Select select =
            sqlParser.parseSelect(
                (MySqlSelectQueryBlock) ((SQLQueryExpr) innerExpr).getSubQuery().getQuery());
  ...
  public static Condition getConditionForMethod(SQLExpr expr, Where.CONN conn)
      throws SqlParseException {
