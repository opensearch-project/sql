...
public class Condition extends Where {
  ...
  public enum OPERATOR {
    ...
    public OPERATOR negative() throws SqlParseException {
      ...
      if (negative == null) {
        throw new SqlParseException(
            StringUtils.format("Negative operator [%s] is not supported.", this.name()));
      }
      return negative;
    }
    ...
    public OPERATOR simpleReverse() throws SqlParseException {
      ...
      if (reverse == null) {
        throw new SqlParseException(
            StringUtils.format("Simple reverse operator [%s] is not supported.", this.name()));
      }
      return reverse;
    }
  ...
  public Condition(
      CONN conn, String field, SQLExpr nameExpr, String condition, Object obj, SQLExpr valueExpr)
      throws SqlParseException {
    this(conn, field, nameExpr, condition, obj, valueExpr, null);
  }

  public Condition(
      CONN conn, String field, SQLExpr nameExpr, OPERATOR condition, Object obj, SQLExpr valueExpr)
      throws SqlParseException {
    this(conn, field, nameExpr, condition, obj, valueExpr, null);
  }
  ...
  /**
   * Return true if the opear is {@link OPERATOR#NESTED_COMPLEX}<br>
   * For example, the opear is {@link OPERATOR#NESTED_COMPLEX} when condition is nested('projects',
   * projects.started_year > 2000 OR projects.name LIKE '%security%')
   */
