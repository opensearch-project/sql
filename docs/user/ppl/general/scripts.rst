...
Script queries are generated when we push down some operators(e.g. `FILTER`, `AGGREGATE`) with functions that are not supported by OpenSearch DSL.
...
Expression tree of ``case(age < 35, 'u35' else email)`` will generate a script query like below. And expression tree will be standardized to ``case(?0 < ?1, ?2 else ?3)`` before encoding, in which ``?i`` indicates the i-th parameter.
...
There are usually 4 parts in the ``params``:

1. ``utcTimestamp``: The timestamp when the query executed, it's used by some time-related functions.
...
3. ``DIGESTS``: The digest of each parameter, it's used as the key to retrieve the value from the corresponding sources. It will be field name for DOC_VALUE and SOURCE, while index in the LITERALS for LITERAL.
4. ``LITERALS``: The extracted literal values in the expression tree.
