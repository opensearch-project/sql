...
public class CalciteTextFunctionIT extends TextFunctionIT {
  ...
  public void testRegexMatch() throws IOException {
    ...
    String query2 = Index.STRINGS.ppl("eval f=regex_match(name, '^he') | fields f");
    ...
    String query3 = Index.STRINGS.ppl("eval f=regex_match(name, 'ld$') | fields f");
    ...
    String query4 = Index.STRINGS.ppl("eval f=regex_match(name, '[hw]o.*d') | fields f");
  ...
  public void testRegexMatchWithWhereClause() throws IOException {
    ...
    String query1 = Index.STRINGS.ppl("where regex_match(name, 'ell') | fields name");
    JSONObject result1 = executeQuery(query1);
    ...
    String query2 = Index.STRINGS.ppl("where regex_match(name, '^h') | fields name");
    JSONObject result2 = executeQuery(query2);
    ...
    String query3 = Index.STRINGS.ppl("where regex_match(name, 'ld$') | fields name");
    JSONObject result3 = executeQuery(query3);
    ...
    String query4 = Index.STRINGS.ppl("where NOT regex_match(name, 'o') | fields name");
    JSONObject result4 = executeQuery(query4);
  ...
  public void testRegexMatchWithComplexPatterns() throws IOException {
    ...
    String query1 =
        Index.STRINGS.ppl("where regex_match(name, '(hello|world)') | fields name | head 3");
    JSONObject result1 = executeQuery(query1);
    verifySchema(result1, schema("name", null, "string"));
    verifyDataRows(result1, rows("hello"), rows("world"), rows("helloworld"));

    // Test regex with word boundary - exact word match
    String query2 = Index.STRINGS.ppl("where regex_match(name, '\\\\bhello\\\\b') | fields name");
    JSONObject result2 = executeQuery(query2);
    verifySchema(result2, schema("name", null, "string"));
    verifyDataRows(result2, rows("hello"));

    // Test regex with quantifiers - at least 5 characters
    String query3 = Index.STRINGS.ppl("where regex_match(name, '^.{5,}$') | fields name");
    JSONObject result3 = executeQuery(query3);
    verifySchema(result3, schema("name", null, "string"));
  ...
  public void testRegexMatchInEvalWithConditions() throws IOException {
    ...
    String query1 =
        Index.STRINGS.ppl(
            "eval category = if(regex_match(name, '^h'), 'starts_with_h', 'other') |"
