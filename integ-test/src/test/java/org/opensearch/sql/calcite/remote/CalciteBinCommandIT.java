...
public class CalciteBinCommandIT extends PPLIntegTestCase {
  ...
  public void testBasicBinWithSpan() throws IOException {
    JSONObject actual = executeQuery(
        String.format("source=%s | bin balance span=5000 | fields account_number, balance, balance_bin | head 5", 
            TEST_INDEX_ACCOUNT));
    ...
    // Verify that binning creates proper buckets (balance values should be rounded down to nearest 5000)
  ...
  public void testBinWithSpanAndAlias() throws IOException {
    JSONObject actual = executeQuery(
        String.format("source=%s | bin balance span=10000 AS balance_range | fields account_number, balance, balance_range | head 3", 
            TEST_INDEX_ACCOUNT));
  ...
  public void testBinWithDecimalSpan() throws IOException {
    JSONObject actual = executeQuery(
        String.format("source=%s | bin balance span=7500.5 AS balance_group | fields account_number, balance, balance_group | head 3", 
            TEST_INDEX_ACCOUNT));
  ...
  public void testBinDefaultBehavior() throws IOException {
    JSONObject actual = executeQuery(
        String.format("source=%s | bin age | fields account_number, age, age_bin | head 3", 
            TEST_INDEX_ACCOUNT));
  ...
  public void testBinIntegrationWithStats() throws IOException {
    JSONObject actual = executeQuery(
        String.format("source=%s | bin balance span=10000 AS balance_bucket | stats count() by balance_bucket | sort balance_bucket", 
            TEST_INDEX_ACCOUNT));
  ...
  public void testBinWithStatsAndAverage() throws IOException {
    JSONObject actual = executeQuery(
        String.format("source=%s | bin balance span=20000 AS balance_group | stats avg(age) as avg_age by balance_group | sort balance_group", 
            TEST_INDEX_ACCOUNT));
  ...
  public void testBinMathematicalCorrectness() throws IOException {
    JSONObject actual = executeQuery(
        String.format("source=%s | bin balance span=15000 | fields account_number, firstname, balance, balance_bin | sort balance | head 3", 
            TEST_INDEX_ACCOUNT));
  ...
  public void testBinWithFilterAndSort() throws IOException {
    JSONObject actual = executeQuery(
        String.format("source=%s | where balance > 30000 | bin balance span=10000 AS balance_bucket | fields firstname, balance, balance_bucket | sort balance_bucket, balance | head 3", 
            TEST_INDEX_ACCOUNT));
  ...
  public void testBinFieldNotFound() {
    ...
    Throwable e = assertThrowsWithReplace(
        IllegalStateException.class, 
        () -> executeQuery(String.format("source=%s | bin nonexistent_field span=1000", TEST_INDEX_ACCOUNT)));
