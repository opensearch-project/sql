...
| `max` | Optional | Truncates the written result to at most N rows, silently dropping the rest. Must be between `1` and `plugins.ppl.outputlookup.max_rows`. |
...
- Overwrite is weak/eventually consistent but gap-free: it writes a fresh slice and atomically repoints the alias, so a concurrent read always sees the whole old slice or the whole new one, never a partial state. The previous slice is left unreferenced.
- Writes to the same lookup are last-writer-wins on the alias and are expected to be serialized by the caller. Concurrent first-time appends converge on a single slice through a deterministic per-lookup discriminant, so every appended row persists. An append that runs alongside an overwrite may write into the slice the overwrite repoints away from, so those appended rows are acknowledged while staying outside the published lookup until a later write includes them. This matches the OpenSearch Dashboards data importer, which uses add-only alias updates and leaves cross-request coordination to the caller.
- The bulk write is not atomic. A partial bulk failure leaves the rows that succeeded in the slice while the query reports the failure, so a retry in append mode without `key_field` appends the succeeded rows a second time. Use `key_field` for an idempotent upsert whose deterministic `_id` makes a retry converge instead of duplicate.
