...
public class SqlRewriteShuttle extends SqlShuttle {
  /**
   * Normalize an identifier by stripping the OpenSearch schema qualifier when present.
   *
   * @param id the identifier to normalize
   * @return a {@code SqlIdentifier} containing only the table name if {@code id} has two parts
   *         and is prefixed with the OpenSearch schema name; otherwise the visit result for {@code id}
   */
  @Override
  ...
  /**
   * Rewrite SQL call nodes to normalize specific patterns before delegation.
   *
   * <p>Performs two targeted rewrites:
   * <ul>
   *   <li>Converts a COUNT() aggregate with no operands into COUNT(*) so that `isCountStar()` and
   *       type derivation behave correctly.</li>
   *   <li>For IN and NOT IN predicates whose first operand is a bare `SqlNodeList`, wraps that list
   *       in a `ROW(...)` call so tuple predicates validate and convert to relational form correctly.</li>
   * </ul>
   *
