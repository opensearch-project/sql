...
When `path` is omitted, the `spath` command runs in auto-extract mode. Instead of extracting a single value, it flattens the entire JSON into a `map<string, any>` column using the following rules:
...
| doc_auto                                                                        | result                                                                           |
|---------------------------------------------------------------------------------+----------------------------------------------------------------------------------|
| {"user":{"name":"John","age":30},"tags":["java","sql"]}                         | {'user.age': 30, 'tags{}': ['java', 'sql'], 'user.name': 'John'}                 |
| {"user":{"name":"Jane","age":25},"tags":["python"]}                             | {'user.age': 25, 'tags{}': 'python', 'user.name': 'Jane'}                        |
