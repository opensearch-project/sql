...
public class IPFunctionIT extends PPLIntegTestCase {
    ...
    public void testCidr() throws IOException {
        ...
        // No matches
        result = executeQuery(String.format("source=%s | where cidr(host, '199.120.111.0/24') | fields url", TEST_INDEX_WEBLOG));
        verifySchema(result, schema("url", null, "boolean"));
        verifyDataRows(result, rows("/shuttle/missions/sts-73/mission-sts-73.html"));

        // One match
        result = executeQuery(String.format("source=%s | where cidr(host, '199.120.110.0/24') | fields url", TEST_INDEX_WEBLOG));
        verifySchema(result, schema("url", null, "boolean"));
        verifyDataRows(result, rows("/shuttle/missions/sts-73/mission-sts-73.html"));
