...
* In case of secure domains, the plugin would only provide access to admin by default for PPL/SQL queries on datasources.
...
======================================
Datasource configuration can be managed using below REST APIs.
...
For eg: with below datasource configuration, only admins and users with prometheus_access role can run queries on my_prometheus datasource. ::
