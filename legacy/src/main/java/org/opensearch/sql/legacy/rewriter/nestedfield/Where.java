...
class Where extends SQLClause<SQLBinaryOpExpr> {
  ...
  /**
   * Rewrite if left and right tag is different (or reach root of WHERE). Otherwise continue
   * delaying the rewrite.
   *
   * <p>Assumption: there are only 2 forms of condition 1) BinaryOp: Left=Identifier, right=value 2)
   * BinaryOp: Left=BinaryOp, right=BinaryOp
   */
  @Override
   ...
   *
   * <p>BinaryOp / \ Identifier Value "employees.age" "30"
   *
   * <p>to
   *
   * <p>BinaryOp / \ Method Value "nested" "30" | Identifier "employees.age"
   *
   * <p>2) For multiple conditions, put entire BinaryOp to the parameter and add function name
   * "nested()" first
   *
   * <p>BinaryOp (a) / \ BinaryOp BinaryOp | | ... ...
   *
   * <p>to
   *
   * <p>Method "nested" | BinaryOp (a) / \ ... ...
   */
