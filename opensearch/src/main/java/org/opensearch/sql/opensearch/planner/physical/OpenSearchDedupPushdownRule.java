...
public class OpenSearchDedupPushdownRule extends RelRule<OpenSearchDedupPushdownRule.Config> {
  ...
  public void onMatch(RelOptRuleCall call) {
    ...
    List<Integer> selectColumns = PlanUtils.getSelectColumns(windows.get(0).partitionKeys);
    String fieldName = fieldNameList.get(selectColumns.get(0));
  ...
  private static boolean validFilter(LogicalFilter filter) {
    ...
    RexNode leftOperand = operandsOfCondition.get(0);
    ...
    String referenceName = filter.getRowType().getFieldNames().get(((RexInputRef) leftOperand).getIndex());
    ...
    RexNode rightOperand = operandsOfCondition.get(1);
    ...
    Integer num = ((RexLiteral) rightOperand).getValueAs(Integer.class);
    if (num == null || num > 1) {
