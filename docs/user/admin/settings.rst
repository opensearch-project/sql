...
When a query aggregates one scan more than once -- ``chart`` and ``timechart`` (the charted rows plus a pass ranking the top-N columns), or ``append``, ``appendcol``, ``multisearch``, ``join`` or a subquery over an aggregating subsearch -- the aggregations are narrowed to a **single shared** subset: the indices aggregatable for the *union* of all their group fields. Narrowing each aggregation independently could pick a different subset for each, so one response would be built from different sets of documents; the shared subset keeps it coherent. If no index is aggregatable for the whole union, the complete result is returned instead. Chained ``stats ... | stats ...`` is unaffected, because the second aggregation reads the first rather than the index.
