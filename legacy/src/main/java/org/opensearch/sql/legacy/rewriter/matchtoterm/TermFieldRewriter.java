...
public class TermFieldRewriter extends MySqlASTVisitorAdapter {
  ...
  public boolean isValidIdentifierForTerm(SQLIdentifierExpr expr) {
    /**
     * Only for following conditions Identifier will be modified Where: WHERE identifier =
     * 'something' IN list: IN ('Tom', 'Dick', 'Harry') IN subquery: IN (SELECT firstname from
     * accounts/account where firstname = 'John') Group by: GROUP BY state , employer , ... Order
     * by: ORDER BY firstname, lastname , ...
     *
     * <p>NOTE: Does not impact fields on ON condition clause in JOIN as we skip visiting
     * SQLJoinTableSource
     */
