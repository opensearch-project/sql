 ...
 * plugin context. A read-only user can read the source but cannot create, write, or alias the
 * destination, so the command must be denied; if it silently ran as a system identity the query
 * would succeed, which would be a privilege escalation.
 */
public class OutputLookupPermissionsIT extends SecurityTestBase {
  ...
  protected void init() throws Exception {
    ...
    if (!initialized) {
      ...
      // Reader role: read + PIT + ppl over ordinary indices.
  ...
  public void readLookupAsAuthorizedUser() throws IOException {
    ...
    // A read-only user reads the lookup through its alias; a plain "*" grant covers the non-hidden
    // backing index (no dot-prefix grant needed), the security benefit of the per-lookup model.
    JSONObject r =
        executeQueryAsUser(
