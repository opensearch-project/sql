  ...
  public void testTimechartWithBy() {
    ...
    String expectedSparkSql =
        "SELECT `@timestamp`, `host`, SUM(`actual_count`) `count`\n"
            ...
            + "FROM `scott`.`events`\n"
            + "GROUP BY `host`, `SPAN`(`@timestamp`, 1, 'm')) `t13`\n"
            + "GROUP BY `@timestamp`) `t14`\n"
            + "CROSS JOIN (SELECT `host`\n"
            + "FROM ((SELECT `host`, SUM(`$f2_0`) `grand_total`\n"
            + "FROM (SELECT `host`, COUNT(*) `$f2_0`\n"
            + "FROM `scott`.`events`\n"
            + "GROUP BY `host`, `SPAN`(`@timestamp`, 1, 'm')) `t17`\n"
            + "GROUP BY `host`\n"
            + "ORDER BY 2 DESC NULLS FIRST\n"
            + "LIMIT 10)\n"
            + "UNION\n"
  ...
  public void testTimechartWithSpan1h() {
    ...
    String expectedSparkSql =
        "SELECT `@timestamp`, `host`, SUM(`actual_count`) `count`\n"
            ...
            + "FROM `scott`.`events`\n"
            + "GROUP BY `host`, `SPAN`(`@timestamp`, 1, 'h')) `t13`\n"
            + "GROUP BY `@timestamp`) `t14`\n"
            + "CROSS JOIN (SELECT `host`\n"
            + "FROM ((SELECT `host`, SUM(`$f2_0`) `grand_total`\n"
            + "FROM (SELECT `host`, COUNT(*) `$f2_0`\n"
            + "FROM `scott`.`events`\n"
            + "GROUP BY `host`, `SPAN`(`@timestamp`, 1, 'h')) `t17`\n"
            + "GROUP BY `host`\n"
            + "ORDER BY 2 DESC NULLS FIRST\n"
            + "LIMIT 10)\n"
            + "UNION\n"
