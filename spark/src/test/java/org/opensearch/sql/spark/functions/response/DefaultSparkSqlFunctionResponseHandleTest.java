...
class DefaultSparkSqlFunctionResponseHandleTest {
    ...
    void testJsonObject() {
        JSONObject jsonObject = new JSONObject("{"
                ...
                + "}"
                + "}");
        DefaultSparkSqlFunctionResponseHandle handle = new DefaultSparkSqlFunctionResponseHandle(jsonObject);
        assertEquals(new ExecutionEngine.Schema(List.of(
        ...
        assertEquals(fromExprValueMap(Map.of("column1", new ExprStringValue("value1"), "column2", new ExprIntegerValue(123), "column3", ExprBooleanValue.of(true))), handle.next());
        assertEquals(fromExprValueMap(Map.of("column1", new ExprStringValue("value2"), "column2", new ExprIntegerValue(456), "column3", ExprBooleanValue.of(false))), handle.next());
    ...
    void testJsonArrayObject() {
        ...
        JSONObject jsonObject = new JSONObject(format("{" + "\"data\": "+
                "     {\n" +
    ...
    void testJsonNestedObject() {
        ...
        DefaultSparkSqlFunctionResponseHandle handle = new DefaultSparkSqlFunctionResponseHandle(jsonObject);
        assertEquals(new ExecutionEngine.Schema(List.of(
                        new ExecutionEngine.Schema.Column("resourceSpans", "resourceSpans", ExprCoreType.STRUCT))),
                handle.schema());
        ...
        assertEquals("scopeSpans",tupleValue.keyValue("resourceSpans").tupleValue().keySet().iterator().next());
        assertEquals("spans",tupleValue.keyValue("resourceSpans").keyValue("scopeSpans").tupleValue().keySet().iterator().next());
        assertEquals(2,tupleValue.tupleValue().values().iterator().next().keyValue("scopeSpans").keyValue("spans").tupleValue().keySet().size());
    ...
    void testJsonNestedObjectArray() {
        ...
        assertEquals("scopeSpans",tupleValue.keyValue("resourceSpans").tupleValue().keySet().iterator().next());
        assertEquals("spans",tupleValue.keyValue("resourceSpans").keyValue("scopeSpans").tupleValue().keySet().iterator().next());
        assertEquals(true,tupleValue.tupleValue().values().iterator().next().keyValue("scopeSpans").keyValue("spans").isArray());
        assertEquals(2,tupleValue.tupleValue().values().iterator().next().keyValue("scopeSpans").keyValue("spans").arrayValue().size());
        assertEquals("attribute",tupleValue.tupleValue().values().iterator().next().keyValue("scopeSpans").keyValue("spans").arrayValue().get(0).tupleValue().keySet().iterator().next());
        assertEquals("attribute",tupleValue.tupleValue().values().iterator().next().keyValue("scopeSpans").keyValue("spans").arrayValue().get(1).tupleValue().keySet().iterator().next());
