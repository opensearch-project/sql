...
public class TemporalUdtRewriteShuttle extends RelHomogeneousShuttle {
  ...
  static RelDataType rewriteType(OpenSearchTypeFactory tf, RelDataType t) {
    ...
    // Recurse into collection types so ARRAY/MULTISET/MAP of temporal types also get UDT-rewritten.
  ...
  private RelNode rebuildProject(Project project) {
    ...
    // Compute output row type from rewritten exps and original field names.
  ...
  private RelNode rebuildCalc(Calc calc) {
    ...
    // Apply the rex shuttle to the program's stored exprs, building a new program in lockstep.
    ...
    // Recreate exprs with rewritten types. Note: program.getExprList() includes the implicit
    // input refs at the head followed by user-defined exprs. Rebuilding via RexProgramBuilder
    // from scratch is simpler than copying.
    // The cleanest approach is to recreate the program from the rewritten exprList, projects, and
    // condition using RexProgramBuilder.create which handles the bookkeeping.
  ...
  private RelNode rebuildValues(Values values) {
    ...
    if (values instanceof LogicalValues) {
      return LogicalValues.create(values.getCluster(), rewritten, values.getTuples());
    }
