...
public class PplSqlToRelConverter extends SqlToRelConverter {
  /**
   * Creates a PplSqlToRelConverter configured for SQL-to-relational conversion with
   * support for Spark-specific join semantics.
   *
   * @param viewExpander    expands views into relational expressions
   * @param validator       SQL validator used during conversion, or {@code null}
   * @param catalogReader   catalog reader for resolving tables and schemas
   * @param cluster         optimization cluster that provides planner context and type factory
   * @param convertletTable convertlet table for SQL-to-Rex conversions
   ...
   *
   * @param root the root of the relational subtree to inspect and potentially modify
   * @param correctJoinType the JoinRelType to apply where a LEFT join should be replaced (e.g., SEMI or ANTI)
   * @return the possibly-modified relational subtree; a new RelNode when a replacement or propagated change occurs, otherwise the original `root`
   */
