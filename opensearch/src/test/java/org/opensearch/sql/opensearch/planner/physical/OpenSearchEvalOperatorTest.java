...
public class OpenSearchEvalOperatorTest {
  ...
  /**
   * The test-case aim to assert OpenSearchEvalOperator behaviour when evaluating geoipFunctionExpression,
   * which is only available when executing on OpeSearch storage engine. (No option)
   */
  @SneakyThrows
  @Test
  public void testEvalOperatorOnGeoIpExpression() {
    ...
    List<Pair<ReferenceExpression, Expression>> ipAddress = List.of(
            ImmutablePair.of(
                    ...
                    new OpenSearchFunctionExpression(BuiltinFunctionName.GEOIP.getName(),
                            List.of(
    ...
    // Make sure generic Expression function as expected when wrapped with OpenSearchEvalOperator.
    Map<String, ExprValue> ipEnrichmentResult = evalOperator.next().keyValue("ipEnrichmentResult").tupleValue();
