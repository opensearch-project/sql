...
public class SortHelper {
  ...
  public static Comparator<ExprValue> constructExprComparator(List<Pair<SortOption, Expression>> sortList) {
    return (o1, o2) -> compareWithExpressions(o1, o2, constructComparator(sortList));
  }

  public static Ordering<ExprValue> constructExprOrdering(List<Pair<SortOption, Expression>> sortList) {
    return Ordering.from(constructExprComparator(sortList));
  }

  private static List<Pair<Expression, Comparator<ExprValue>>> constructComparator(List<Pair<SortOption, Expression>> sortList) {
  ...
  private static int compareWithExpressions(ExprValue o1, ExprValue o2, List<Pair<Expression, Comparator<ExprValue>>> comparators) {
    for (Pair<Expression, Comparator<ExprValue>> comparator : comparators) {
      ...
      int result = comparator.getValue().compare(
