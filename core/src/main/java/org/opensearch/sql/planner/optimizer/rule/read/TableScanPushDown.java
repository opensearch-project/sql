...
/**
 * Rule template for all table scan push down rules. Because all push down optimization rules have
 * similar workflow in common, such as a pattern that match an operator on top of table scan
 * builder, and action that eliminates the original operator if pushed down, this class helps remove
 * redundant code and improve readability.
 *
...
public class TableScanPushDown<T extends LogicalPlan> implements Rule<T> {
  ...
  private TableScanPushDown(
      WithPattern<T> pattern, BiFunction<T, TableScanBuilder, Boolean> pushDownFunction) {
