...
public class TransportCreateDataSourceAction
  ...
  protected void doExecute(Task task, CreateDataSourceActionRequest request,
    ...
    actionListener.onResponse(execute(request.getDataSourceMetadata()));
  }

  private CreateDataSourceActionResponse execute(DataSourceMetadata dataSourceMetadata) {
    dataSourceService.createDataSource(dataSourceMetadata);
    return new CreateDataSourceActionResponse("Created DataSource with name "
