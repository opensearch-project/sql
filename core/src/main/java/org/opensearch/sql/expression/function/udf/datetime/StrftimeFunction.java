...
public class StrftimeFunction extends ImplementorUDF {
  ...
  // Maximum valid UNIX timestamp (same as FROM_UNIXTIME)
  private static final long MAX_UNIX_TIMESTAMP = 32536771199L;
  private static final long MIN_MILLISECONDS = 100000000000L;
  ...
  private static Double extractUnixSecondsWithFraction(ExprValue unixTime) {
    ...
    // Note: We only accept numeric types, NOT strings
    if (!(unixTime instanceof ExprLongValue)
        && !(unixTime instanceof ExprDoubleValue)
        && !(unixTime instanceof ExprIntegerValue)
        && !(unixTime instanceof ExprFloatValue)) {
    ...
    // If it's in the valid seconds range, treat as seconds with optional fractional part
    if (numericValue <= MAX_UNIX_TIMESTAMP) {
      return numericValue;
    }

    // For larger values, auto-detect milliseconds (divide by 1000)
    // This handles epoch_millis fields from OpenSearch
    if (numericValue < MIN_MILLISECONDS) {
      // Value between MAX_UNIX_TIMESTAMP and MIN_MILLISECONDS is ambiguous
      return null;
    }

    // Convert milliseconds to seconds (preserving fractional part)
  ...
  private static boolean isValidTimestamp(long unixSeconds) {
    return unixSeconds >= 0 && unixSeconds <= MAX_UNIX_TIMESTAMP;
  }
  ...
  private static ZonedDateTime convertToZonedDateTimeWithFraction(double unixSeconds) {
    ...
    int nanos = (int) ((unixSeconds - seconds) * 1_000_000_000);
