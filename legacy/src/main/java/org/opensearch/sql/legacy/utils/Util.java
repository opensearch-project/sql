...
public class Util {
  ...
  public static boolean isFromJoinOrUnionTable(SQLExpr expr) {
    ...
    while (temp != null
        ...
        && !(expr instanceof SQLJoinTableSource)
        && !(expr instanceof SQLUnionQuery)
        && counter.get() > 0) {
  ...
  public static GetIndexRequestBuilder prepareIndexRequestBuilder(
      Client client, IndexStatement statement) {
    /*
     ...
     */
    GetIndexRequestBuilder indexRequestBuilder =
        client
            .admin()
            .indices()
            .prepareGetIndex()
