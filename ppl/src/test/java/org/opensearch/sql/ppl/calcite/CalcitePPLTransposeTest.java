  ...
  public void testSimpleCountWithTranspose() {
    ...
    String expectedLogical =
        "LogicalAggregate(group=[{0}], row 1=[MAX($1)], row 2=[MAX($2)], row 3=[MAX($3)], row"
            ...
            + " CAST($0):VARCHAR NOT NULL, null:NULL)])\n"
    ...
    String expectedSparkSql =
        "SELECT `column`, MAX(CASE WHEN `__row_id__` = 1 THEN CAST(`value` AS STRING) ELSE NULL"
            ...
            + " END) `row 5`\n"
            + "FROM (SELECT `t0`.`c`, `t0`.`__row_id__`, `t1`.`column`, CASE WHEN `t1`.`column` ="
            + " 'c' THEN CAST(`t0`.`c` AS STRING) ELSE NULL END `value`\n"
  ...
  public void testMultipleAggregatesWithAliasesTranspose() {
    ...
    String expectedLogical =
        "LogicalAggregate(group=[{0}], row 1=[MAX($1)], row 2=[MAX($2)], row 3=[MAX($3)], row"
            ...
            + " __row_id__=[$4], column=[$5], value=[CASE(=($5, 'avg_sal'), NUMBER_TO_STRING($0),"
            + " =($5, 'max_sal'), NUMBER_TO_STRING($1), =($5, 'min_sal'), NUMBER_TO_STRING($2),"
            + " =($5, 'cnt'), CAST($3):VARCHAR NOT NULL, null:NULL)])\n"
            + "        LogicalJoin(condition=[true], joinType=[inner])\n"
            + "          LogicalProject(avg_sal=[$0], max_sal=[$1], min_sal=[$2], cnt=[$3],"
  ...
  public void testTransposeWithLimit() {
    ...
    String expectedLogical =
        "LogicalAggregate(group=[{0}], row 1=[MAX($1)], row 2=[MAX($2)], row 3=[MAX($3)])\n"
            ...
            + " NUMBER_TO_STRING($1), =($5, 'JOB'), CAST($2):VARCHAR NOT NULL, =($5, 'SAL'),"
            + " NUMBER_TO_STRING($3), null:NULL)])\n"
            + "        LogicalJoin(condition=[true], joinType=[inner])\n"
    ...
    String expectedSparkSql =
        "SELECT `column`, MAX(CASE WHEN `__row_id__` = 1 THEN CAST(`value` AS STRING) ELSE NULL"
            ...
            + " END) `row 2`, MAX(CASE WHEN `__row_id__` = 3 THEN CAST(`value` AS STRING) ELSE NULL"
            + " END) `row 3`\n"
            + "FROM (SELECT `t`.`ENAME`, `t`.`COMM`, `t`.`JOB`, `t`.`SAL`, `t`.`__row_id__`,"
            + " `t0`.`column`, CASE WHEN `t0`.`column` = 'ENAME' THEN CAST(`t`.`ENAME` AS STRING)"
            + " WHEN `t0`.`column` = 'COMM' THEN NUMBER_TO_STRING(`t`.`COMM`) WHEN `t0`.`column` ="
            + " 'JOB' THEN CAST(`t`.`JOB` AS STRING) WHEN `t0`.`column` = 'SAL' THEN"
  ...
  public void testTransposeWithLimitColumnName() {
    ...
    String expectedLogical =
        "LogicalAggregate(group=[{0}], row 1=[MAX($1)], row 2=[MAX($2)], row 3=[MAX($3)])\n"
            ...
            + " 'COMM'), NUMBER_TO_STRING($1), =($5, 'JOB'), CAST($2):VARCHAR NOT NULL, =($5,"
            + " 'SAL'), NUMBER_TO_STRING($3), null:NULL)])\n"
            + "        LogicalJoin(condition=[true], joinType=[inner])\n"
            + "          LogicalProject(ENAME=[$1], COMM=[$6], JOB=[$2], SAL=[$5],"
            + " __row_id__=[ROW_NUMBER() OVER ()])\n"
            + "            LogicalTableScan(table=[[scott, EMP]])\n"
    ...
    String expectedSparkSql =
        "SELECT `column_names`, MAX(CASE WHEN `__row_id__` = 1 THEN CAST(`value` AS STRING) ELSE"
            + " NULL END) `row 1`, MAX(CASE WHEN `__row_id__` = 2 THEN CAST(`value` AS STRING) ELSE"
            + " NULL END) `row 2`, MAX(CASE WHEN `__row_id__` = 3 THEN CAST(`value` AS STRING) ELSE"
            + " NULL END) `row 3`\n"
            + "FROM (SELECT `t`.`ENAME`, `t`.`COMM`, `t`.`JOB`, `t`.`SAL`, `t`.`__row_id__`,"
            + " `t0`.`column_names`, CASE WHEN `t0`.`column_names` = 'ENAME' THEN CAST(`t`.`ENAME`"
            + " AS STRING) WHEN `t0`.`column_names` = 'COMM' THEN NUMBER_TO_STRING(`t`.`COMM`) WHEN"
            + " `t0`.`column_names` = 'JOB' THEN CAST(`t`.`JOB` AS STRING) WHEN `t0`.`column_names`"
            + " = 'SAL' THEN NUMBER_TO_STRING(`t`.`SAL`) ELSE NULL END `value`\n"
            + "FROM (SELECT `ENAME`, `COMM`, `JOB`, `SAL`, ROW_NUMBER() OVER () `__row_id__`\n"
            + "FROM `scott`.`EMP`) `t`\n"
            + "CROSS JOIN (VALUES ('ENAME'),\n"
            + "('COMM'),\n"
