...
class OpenSearchQueryManagerTest {
  ...
  @Test
  public void carriesParentMarkerHeaderAcrossPoolHop() {
    // Mock shares one context across caller and worker, so this exercises the withCurrentContext
    // putHeader path without a real thread hop.
    ThreadContext ctx = new ThreadContext(EMPTY);
    ctx.putHeader("X-Query-Insights-Parent", "PPL:node-1:99");
    ...
    when(nodeClient.threadPool()).thenReturn(threadPool);
    when(threadPool.getThreadContext()).thenReturn(ctx);
    when(settings.getSettingValue(Settings.Key.PPL_QUERY_TIMEOUT))
        .thenReturn(TimeValue.timeValueSeconds(60));
    ...
    assertTrue(ran.get());
    assertEquals("PPL:node-1:99", ctx.getHeader("X-Query-Insights-Parent"));
  }
