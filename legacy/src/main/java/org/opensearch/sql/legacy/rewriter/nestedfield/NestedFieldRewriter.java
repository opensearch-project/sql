...
/**
 * Visitor to rewrite AST (abstract syntax tree) for nested type fields to support implicit nested()
 * function call. Intuitively, the approach is to implement SQLIdentifier.visit() and wrap nested()
 * function for nested field. The parsing result of FROM clause will be used to determine if an
 * identifier is nested field.
 *
 * <p>State transition table (state here means Scope + Query AST):
 * _________________________________________________________________________________________________________________
 * | Rewrite | Scope | Sample Query |
 * |---------------------------------------------------------------------------------------------------------------|
 * | (Start) | () | SELECT e.lastname, COUNT(*) FROM team t, employees e | | | | WHERE region = 'US'
 * and e.firstname = 'John' | | | | GROUP BY e.lastname |
 * |---------------------------------------------------------------------------------------------------------------|
 * | FROM | (parentAlias='t' | SELECT e.lastname, COUNT(*) FROM team | | | aliasFullPaths={e:
 * employees}| WHERE region = 'US' and e.firstname = 'John' | | | conditionTags={}) | GROUP BY
 * e.lastname |
 * |---------------------------------------------------------------------------------------------------------------|
 ...
 * |---------------------------------------------------------------------------------------------------------------|
 *
 * <p>Note 'c' in conditionTag refer to the reference to SQLBinaryOpExpr object of condition
 * 'employees.firstname = 'John'
 *
 * <p>More details: 1) Manage environment in the case of subquery 2) Add nested field to select for
 * SELECT * 3) Merge conditions of same nested field to single nested() call
 */
