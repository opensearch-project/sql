...
public class BinAnalysisIT extends PPLIntegTestCase {
  ...
  public void analyzeAccountsData() throws IOException {
    ...
    // Sample data
    JSONObject sample = executeQuery(String.format("source=%s | fields account_number, balance, age | head 3", TEST_INDEX_ACCOUNT));
    System.out.println("Sample data: " + sample.toString());
    
    // Balance range
    JSONObject balanceRange = executeQuery(String.format("source=%s | stats min(balance) as min_bal, max(balance) as max_bal", TEST_INDEX_ACCOUNT));
  ...
  public void testBinBasic() throws IOException {
    ...
    JSONObject binBalance = executeQuery(String.format("source=%s | bin balance span=5000 | fields account_number, balance, balance_bin | head 3", TEST_INDEX_ACCOUNT));
    System.out.println("Bin balance span=5000: " + binBalance.toString());
    
    // Test simple bin with age
    JSONObject binAge = executeQuery(String.format("source=%s | bin age span=5 | fields account_number, age, age_bin | head 3", TEST_INDEX_ACCOUNT));
  ...
  public void testBinWithStats() throws IOException {
    ...
    JSONObject statsResult = executeQuery(String.format("source=%s | bin balance span=10000 AS bucket | stats count() by bucket | sort bucket", TEST_INDEX_ACCOUNT));
    System.out.println("Balance histogram (span=10000): " + statsResult.toString());
    
    // Age histogram
    JSONObject ageHisto = executeQuery(String.format("source=%s | bin age span=5 AS age_group | stats count() by age_group | sort age_group", TEST_INDEX_ACCOUNT));
  ...
  public void testBinsBug() throws IOException {
    ...
    JSONObject binsResult = executeQuery(String.format("source=%s | bin age bins=5 | fields account_number, age, age_bin | head 3", TEST_INDEX_ACCOUNT));
    System.out.println("Bins parameter result: " + binsResult.toString());
    
    // Get age data for manual calculation
    JSONObject ageStats = executeQuery(String.format("source=%s | stats min(age) as min_age, max(age) as max_age, count() as cnt", TEST_INDEX_ACCOUNT));
