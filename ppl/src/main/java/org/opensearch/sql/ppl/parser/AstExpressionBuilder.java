...
public class AstExpressionBuilder extends OpenSearchPPLParserBaseVisitor<UnresolvedExpression> {
  ...
  public UnresolvedExpression visitTableFunction(OpenSearchPPLParser.TableFunctionContext ctx) {
    ...
    ctx.tableFunctionArgument().forEach(arg
      ...
      if(arg.tableFunctionArgValue().RAWQUERY() != null) {
        builder.add(
            new UnresolvedArgument(argName, new Literal(StringUtils.getQueryText(arg.tableFunctionArgValue().getText()), DataType.STRING)));
      }
      else {
        builder.add(
            new UnresolvedArgument(argName, this.visitTableFunctionArgValue(arg.tableFunctionArgValue())));
      }
    });
