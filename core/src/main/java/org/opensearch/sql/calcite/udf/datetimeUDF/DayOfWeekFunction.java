...
import static org.opensearch.sql.expression.datetime.DateTimeFunctions.dayOfWeekToday;
import static org.opensearch.sql.expression.datetime.DateTimeFunctions.exprDayOfWeek;
...
public class DayOfWeekFunction implements UserDefinedFunction {
  ...
  public ExprValue extractForTime(ExprValue candidate, FunctionProperties functionProperties) {
    return dayOfWeekToday(functionProperties.getQueryStartClock());
  }

  public ExprValue extract(ExprValue candidate) {
    return exprDayOfWeek(candidate);
  }
}
