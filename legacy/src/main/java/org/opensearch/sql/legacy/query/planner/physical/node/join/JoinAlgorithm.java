...
public abstract class JoinAlgorithm<T> extends BatchPhysicalOperator<T> {
   ...
   * join).
   *
   * <p>1) Build hash table and open right side. 2) Keep probing right to find matched rows
   * (meanwhile update mismatched set) 3) Check if any row in mismatched set to return in the case
   * of outer join. 4) Nothing remained now, move on to next block of left. Go back to step 1.
   *
   * <p>This is a new run AND no block from left means algorithm should stop and return empty.
   */
  @Override
  ...
  /** Building phase: Build hash table from data block. */
