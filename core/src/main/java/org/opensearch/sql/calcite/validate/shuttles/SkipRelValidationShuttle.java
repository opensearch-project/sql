...
public class SkipRelValidationShuttle extends RelShuttleImpl {
  ...
  /**
   * Visits a LogicalAggregate and marks validation to be skipped when any configured predicate matches.
   *
   * @param aggregate the aggregate node to inspect
   * @return the original {@code aggregate} if a skip predicate matched and skipping was enabled; otherwise the visited aggregate node returned by the superclass
   */
  @Override
  ...
  /**
   * Visits a LogicalValues node and marks validation to be skipped when it matches any configured skip predicate.
   *
   * @param values LogicalValues node to inspect for skip conditions
   * @return the original {@code values} node if a skip predicate matches (and skipping is enabled), otherwise the result of visiting the node via the superclass
   */
  @Override
  ...
  /**
   * Initializes the SkipRelValidationShuttle and installs a RexShuttle that detects skip conditions within RexCall nodes.
   *
   * <p>The installed RexShuttle tests each visited {@code RexCall} against the {@code SKIP_CALLS} predicates;
  ...
  /**
   * Indicates whether traversal detected conditions that should bypass validation.
   *
  ...
  /**
   * Visits the specified child relational node and applies the shuttle for expression-level checks.
   *
   * @param parent the parent relational node containing the child
   * @param i      the index of the child within the parent
   * @param child  the child relational node to visit
   * @return       the child node after visiting and applying the internal RexShuttle (may be unchanged)
   */
  @Override
