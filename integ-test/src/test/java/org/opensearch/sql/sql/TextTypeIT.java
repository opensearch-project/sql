...
public class TextTypeIT extends SQLIntegTestCase {
  ...
  public void test_all_data_returned_as_strings() {
    ...
    verifyDataRows(result,
        ...
        rows(null, null, "kokoko", "A fairy tale about little mermaid", "A Text with a Field", null, null)
    );
  }

  @Test
  // TODO uncomment test ascii on TextWithKeywords (after https://github.com/Bit-Quill/opensearch-project-sql/pull/301)
  public void test_ascii() {
    //JSONObject result = executeQuery("select ascii(Keyword), ascii(TextWithKeywords), ascii(Text), ascii(TextWithFielddata), "
    JSONObject result = executeQuery("select ascii(Keyword), ascii(Text), ascii(TextWithFielddata), "
        + "ascii(TextWithFields), ascii(TextWithNumbers), ascii(TextWithMixedFields) FROM " + TEST_INDEX_TEXT);
  ...
  public void test_concat_with_user_string() {
    JSONObject result = executeQuery("select concat(Keyword), concat(Keyword, 'ab'), "
        ...
        + "concat(TextWithMixedFields), concat(TextWithMixedFields, 'fg') FROM " + TEST_INDEX_TEXT);
    ...
    verifyDataRows(result,
        rows("K for Keyword", "K for Keywordab", "Text With Keywords", "Text With Keywordsbc", null, null,
            "Text", "Textcd", null, null, null, null, "one two tree", "one two treefg"),
        rows(null, null, null, null, "Text", "Textbc", "Fielddata is the best!", "Fielddata is the best!cd",
            "Fields for freedom", "Fields for freedomde", "42", "42ef", "42", "42fg"),
        rows("ikiki", "ikikiab", "Another Text With Similar Keywords", "Another Text With Similar Keywordsbc",
            null, null, null, null, null, null, "42", "42ef", "12.04.1961", "12.04.1961fg"),
        rows(null, null, "", "bc", "Some text", "Some textbc", "Text With Fielddata", "Text With Fielddatacd",
            "Freedom for fields", "Freedom for fieldsde", "100500", "100500ef", null, null),
        rows("kokoko", "kokokoab", null, null, "A fairy tale about little mermaid", "A fairy tale about little mermaidbc",
            null, null, "A Text with a Field", "A Text with a Fieldde", null, null, null, null)
    );
  }

  @Test
  public void test_concat_ws_with_different_types() {
    JSONObject result = executeQuery("select concat_ws(' ', Keyword, Keyword) AS `KW + KW`, "
        ...
        + "concat_WS(' == ', TextWithMixedFields, TextWithNumbers) AS `T_MX + T_N` FROM " + TEST_INDEX_TEXT);
    ...
    verifyDataRows(result,
        ...
        rows("ikiki ikiki", "Another Text With Similar Keywords ikiki", null, "|/42", "12.04.1961 == 42"),
  ...
  public void test_simple_string_functions() {
    JSONObject result = executeQuery("select left(Keyword, 4) as left, upper(TextWithKeywords) as upper, "
        + "locate('t', Text) as locate, trim(TextWithFielddata) as trim, "
        + "reverse(TextWithFields) as reverse, substring(TextWithNumbers, 5, 3) as substring, "
        + "TextWithMixedFields = '42' as compare FROM " + TEST_INDEX_TEXT);
  ...
  public void test_cast_text_to_bool() {
    JSONObject result = executeQuery(String.format("select "
  ...
  public void test_cast_text_to_int() {
    JSONObject result = executeQuery(String.format("select "
        ...
        + "from %s where TestName = 'int';", TEST_INDEX_TEXT_FOR_CAST));
  ...
  public void test_cast_text_to_double() {
    JSONObject result = executeQuery(String.format("select "
        ...
        + "from %s where TestName = 'double';", TEST_INDEX_TEXT_FOR_CAST));
    ...
    verifyDataRows(result,
        rows(1.0, 3.14, -333.0, 42.0, .0, .223)
    );
  }
  ...
  public void test_cast_text_to_date() {
    ...
    verifyDataRows(result,
        rows("1984-04-04", "1999-10-20", "1977-07-17", "1961-04-12", "2020-02-29")
    );
  }
  ...
  public void test_cast_text_to_time() {
    ...
    JSONObject result = executeQuery(String.format("select "
        ...
        + "from %s where TestName = 'time';", TEST_INDEX_TEXT_FOR_CAST));
    ...
    verifyDataRows(result,
        rows("22:22:00", "00:00:00", "07:17:00", "10:20:30", "00:00:01")
    );
  }
  ...
  public void test_cast_text_to_timestamp() {
    ...
    JSONObject result = executeQuery(String.format("select "
        ...
        + "from %s where TestName = 'timestamp';", TEST_INDEX_TEXT_FOR_CAST));
    ...
    verifyDataRows(result,
        rows("1984-04-04 10:20:30", "1999-10-20 00:01:02", "1977-07-17 01:02:03",
            "1961-04-12 09:07:00", "2020-02-29 00:00:00")
    );
  }
