...
def convert_rst_table_to_markdown(table_lines):
...
def convert_heading(line, next_line, heading_map):
...
def parse_os_block(block_content):
...
def split_multiple_queries(block_content):
...
def convert_code_block_to_markdown(block_content):
...
def convert_rst_to_markdown(rst_content):
    ...
    i = 0
    skip_next = False
    heading_map = {}  # Track underline char to heading level mapping
...
def main():
    ...
    if sys.argv[1] == '--batch':
        ...
        directory = sys.argv[2] if len(sys.argv) > 2 else 'docs/user/ppl/cmd'
        
        rst_files = list(Path(directory).glob('*.rst'))
        print(f"Found {len(rst_files)} RST files in {directory}")
        
        for rst_file in rst_files:
            md_file = rst_file.with_suffix('.md')
            try:
                convert_file(str(rst_file), str(md_file))
            except Exception as e:
                print(f"  ✗ Error: {e}")
    else:
        # Single file conversion
        rst_file = sys.argv[1]
        md_file = sys.argv[2] if len(sys.argv) > 2 else rst_file.replace('.rst', '.md')
