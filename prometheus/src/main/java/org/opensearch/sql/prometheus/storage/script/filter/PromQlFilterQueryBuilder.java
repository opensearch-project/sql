...
public class PromQlFilterQueryBuilder extends ExpressionNodeVisitor<PromFilterQuery, Object> {
  ...
  public PromFilterQuery visitFunction(FunctionExpression func, Object context) {
    ...
    if (!StringUtils.isEmpty(promFilterQuery.getPromQl())) {
      promFilterQuery.setPromQl(promFilterQuery.getPromQl().insert(0, "{").append("}"));
    }
    return promFilterQuery;
  }

  private void visitFunction(PromFilterQuery promFilterQuery, FunctionExpression func,
                             Object context) {
