...
public class CalciteStreamstatsCommandIT extends PPLIntegTestCase {
  ...
  public void testStreamstatsBy() throws IOException {
    JSONObject actual =
        executeQuery(
            String.format(
                "source=%s | streamstats count() as cnt, avg(age) as avg, min(age) as min, max(age)"
                    + " as max by country | fields name, country, state, month, year, age, cnt, avg, min, max",
                TEST_INDEX_STATE_COUNTRY));
  ...
  public void testStreamstatsByWithNull() throws IOException {
    ...
    actual =
        executeQuery(
            String.format(
                "source=%s | streamstats count() as cnt, avg(age) as avg, min(age) as min, max(age)"
                    + " as max by state | fields name, country, state, month, year, age, cnt, avg, min, max",
                TEST_INDEX_STATE_COUNTRY_WITH_NULL));
    verifyDataRows(
        actual,
        rows("Jake", "USA", "California", 4, 2023, 70, 1, 70, 70, 70),
  ...
  public void testStreamstatsByWithNullBucket() throws IOException {
        ...
        executeQuery(
            String.format(
                "source=%s | streamstats bucket_nullable=false count() as cnt, avg(age) as avg,"
                    + " min(age) as min, max(age) as max by country | fields name, country, state, month, year, age, cnt, avg, min, max",
                TEST_INDEX_STATE_COUNTRY_WITH_NULL));
    ...
    actual =
        executeQuery(
            String.format(
                "source=%s | streamstats bucket_nullable=false count() as cnt, avg(age) as avg,"
                    + " min(age) as min, max(age) as max by state | fields name, country, state, month, year, age, cnt, avg, min, max",
                TEST_INDEX_STATE_COUNTRY_WITH_NULL));
    verifyDataRows(
        actual,
        rows("Jake", "USA", "California", 4, 2023, 70, 1, 70, 70, 70),
  ...
  public void testStreamstatsBySpan() throws IOException {
    JSONObject actual =
        executeQuery(
            String.format(
                "source=%s | streamstats count() as cnt, avg(age) as avg, min(age) as min, max(age)"
                    + " as max by span(age, 10) as age_span | fields name, country, state, month, year, age, cnt, avg, min, max",
                TEST_INDEX_STATE_COUNTRY));
  ...
  public void testStreamstatsByMultiplePartitions1() throws IOException {
    JSONObject actual =
        executeQuery(
            String.format(
                "source=%s | streamstats count() as cnt, avg(age) as avg, min(age) as min, max(age)"
                    + " as max by span(age, 10) as age_span, country | fields name, country, state, month, year, age, cnt, avg, min, max",
                TEST_INDEX_STATE_COUNTRY));
  ...
  public void testStreamstatsByMultiplePartitions2() throws IOException {
    JSONObject actual =
        executeQuery(
            String.format(
                "source=%s | streamstats count() as cnt, avg(age) as avg, min(age) as min, max(age)"
                    + " as max by span(age, 10) as age_span, state | fields name, country, state, month, year, age, cnt, avg, min, max",
                TEST_INDEX_STATE_COUNTRY));
  ...
  public void testStreamstatsCurrent() throws IOException {
    JSONObject actual =
        executeQuery(
            String.format(
                "source=%s | streamstats current=false avg(age) as prev_avg | fields name, country, state, month, year, age, prev_avg",
                TEST_INDEX_STATE_COUNTRY));
  ...
  public void testStreamstatsWindow() throws IOException {
    JSONObject actual =
        executeQuery(
            String.format(
                "source=%s | streamstats window = 3 avg(age) as avg | fields name, country, state, month, year, age, avg", TEST_INDEX_STATE_COUNTRY));
  ...
  public void testStreamstatsWindowWithNull() throws IOException {
    JSONObject actual =
        executeQuery(
            String.format(
                "source=%s | streamstats window = 3 avg(age) as avg | fields name, country, state, month, year, age, avg",
                TEST_INDEX_STATE_COUNTRY_WITH_NULL));
  ...
  public void testStreamstatsBigWindow() throws IOException {
    JSONObject actual =
        executeQuery(
            String.format(
                "source=%s | streamstats window = 10 avg(age) as avg | fields name, country, state, month, year, age, avg", TEST_INDEX_STATE_COUNTRY));
  ...
  public void testStreamstatsCurrentAndWindow() throws IOException {
    JSONObject actual =
        executeQuery(
            String.format(
                "source=%s | streamstats current = false window = 2 avg(age) as avg | fields name, country, state, month, year, age, avg",
                TEST_INDEX_STATE_COUNTRY));
  ...
  public void testStreamstatsGlobal() throws IOException {
    ...
    try {
      JSONObject actual =
          executeQuery(
              String.format(
                  "source=%s | streamstats window=2 global=false avg(age) as avg by country | fields name, country, state, month, year, age, avg",
                  TEST_INDEX_STATE_COUNTRY));
      ...
      JSONObject actual2 =
          executeQuery(
              String.format(
                  "source=%s | streamstats window=2 global=true avg(age) as avg by country | fields name, country, state, month, year, age, avg",
                  TEST_INDEX_STATE_COUNTRY));
  ...
  public void testStreamstatsReset() throws IOException {
    ...
    try {
      JSONObject actual =
          executeQuery(
              String.format(
                  "source=%s | streamstats window=2 reset_before=age>29 avg(age) as avg by country | fields name, country, state, month, year, age, avg",
                  TEST_INDEX_STATE_COUNTRY));
      ...
      JSONObject actual2 =
          executeQuery(
              String.format(
                  "source=%s | streamstats window=2 reset_after=age>22 avg(age) as avg by country | fields name, country, state, month, year, age, avg",
                  TEST_INDEX_STATE_COUNTRY));
          ...
          executeQuery(
              String.format(
                  "source=%s | streamstats bucket_nullable=true window=2 reset_before=age>29"
                      + " avg(age) as avg by state | fields name, country, state, month, year, age, avg",
                  TEST_INDEX_STATE_COUNTRY_WITH_NULL));
        ...
        executeQuery(
            String.format(
                "source=%s | streamstats avg(age) as avg_age by state, country | streamstats"
                    + " avg(avg_age) as avg_state_age by country | fields name, country, state, month, year, age, avg_age, avg_state_age",
                TEST_INDEX_STATE_COUNTRY));
        ...
        executeQuery(
            String.format(
                "source=%s | eventstats avg(age) as avg_age| streamstats"
                    + " avg(age) as avg_age_stream | fields name, country, state, month, year, age, avg_age, avg_age_stream",
                TEST_INDEX_STATE_COUNTRY));
  ...
  public void testStreamstatsAndSort() throws IOException {
    JSONObject actual =
        executeQuery(
            String.format(
                "source=%s | sort age | streamstats window = 2 avg(age) as avg_age | fields name, country, state, month, year, age, avg_age",
                TEST_INDEX_STATE_COUNTRY));
        ...
        executeQuery(
            String.format(
                "source=%s as l | left join left=l right=r on l.country = r.country [ source=%s |"
                    + " streamstats window=2 avg(age) as avg_age] | fields l.name, l.country, l.state,"
                    + " l.month, l.year, l.age, r.name, r.country, r.state, r.month, r.year, r.age, avg_age",
        ...
        executeQuery(
            String.format(
                "source=%s | where country in [ source=%s | streamstats window=2 avg(age) as"
                    + " avg_age | where avg_age > 40 | fields country ] | fields name, country, state, month, year, age",
                TEST_INDEX_STATE_COUNTRY, TEST_INDEX_STATE_COUNTRY_WITH_NULL));
        ...
        executeQuery(
            String.format(
                "source=%s | eval new_state=lower(state), new_country=lower(country) | streamstats"
                    ...
                    + " fields name, country, state, month, year, age, new_state, new_country, avg_age",
                TEST_INDEX_STATE_COUNTRY_WITH_NULL));
  ...
  public void testStreamstatsDistinctCount() throws IOException {
    JSONObject actual =
        executeQuery(
            String.format(
                "source=%s | streamstats dc(state) as dc_state | fields name, country, state, month, year, age, dc_state", TEST_INDEX_STATE_COUNTRY));
  ...
  public void testStreamstatsDistinctCountByCountry() throws IOException {
    JSONObject actual =
        executeQuery(
            String.format(
                "source=%s | streamstats dc(state) as dc_state by country | fields name, country, state, month, year, age, dc_state",
                TEST_INDEX_STATE_COUNTRY));
  ...
  public void testStreamstatsDistinctCountFunction() throws IOException {
    JSONObject actual =
        executeQuery(
            String.format(
                "source=%s | streamstats distinct_count(country) as dc_country | fields name, country, state, month, year, age, dc_country",
                TEST_INDEX_STATE_COUNTRY));
  ...
  public void testStreamstatsDistinctCountWithNull() throws IOException {
    JSONObject actual =
        executeQuery(
            String.format(
                "source=%s | streamstats dc(state) as dc_state | fields name, country, state, month, year, age, dc_state",
                TEST_INDEX_STATE_COUNTRY_WITH_NULL));
