...
public class BinCommandDebugIT extends PPLIntegTestCase {
  ...
  public void step1_verifyAccountsDataLoaded() throws IOException {
    ...
    JSONObject result =
        executeQuery(
            String.format(
                "source=%s | fields account_number, balance, age | head 5", TEST_INDEX_ACCOUNT));
  ...
  public void step2_testBasicBinCommand() throws IOException {
    ...
    try {
      JSONObject result =
          executeQuery(
              String.format(
                  "source=%s | bin age span=5 | fields account_number, age, age_bin | head 5",
                  TEST_INDEX_ACCOUNT));
  ...
  public void step3_testBinWithAlias() throws IOException {
    ...
    try {
      JSONObject result =
          executeQuery(
              String.format(
                  "source=%s | bin balance span=10000 AS balance_bucket | fields account_number,"
                      + " balance, balance_bucket | head 3",
                  TEST_INDEX_ACCOUNT));
  ...
  public void step6_analyzeDataRanges() throws IOException {
    ...
    // Get min/max balance
    try {
      JSONObject minBalance =
          executeQuery(
              String.format("source=%s | stats min(balance) as min_balance", TEST_INDEX_ACCOUNT));
      System.out.println("Min balance: " + minBalance.toString());

      JSONObject maxBalance =
          executeQuery(
              String.format("source=%s | stats max(balance) as max_balance", TEST_INDEX_ACCOUNT));
  ...
  public void step8_testDecimalSpan() throws IOException {
    ...
    try {
      JSONObject result =
          executeQuery(
              String.format(
                  "source=%s | bin balance span=2500.5 AS balance_group | fields account_number,"
