...
* Support bi-directional graph traversal with the `graphlookup` PPL command using breadth-first search ([#5138](https://github.com/opensearch-project/sql/pull/5138))
* Support `graphlookup` as a top-level PPL command with literal start values ([#5253](https://github.com/opensearch-project/sql/pull/5253))
* Add `mvexpand` PPL command to expand multivalue fields into separate rows ([#5144](https://github.com/opensearch-project/sql/pull/5144))
* Add `fieldformat` PPL command to set display format of field values using eval expressions ([#5080](https://github.com/opensearch-project/sql/pull/5080))
* Add `convert` PPL command with five conversion functions: auto, num, rmcomma, rmunit, and none ([#5157](https://github.com/opensearch-project/sql/pull/5157))
* Add PPL search result highlighting via the `highlight` API parameter ([#5234](https://github.com/opensearch-project/sql/pull/5234))
...
* Change `graphlookup` struct output from list to map to preserve column name information ([#5227](https://github.com/opensearch-project/sql/pull/5227))
* Preserve UDT types through UNION operations to fix multisearch type loss for span, bin, and stats ([#5154](https://github.com/opensearch-project/sql/pull/5154))

### Enhancements

* Add auto-extract mode for `spath` command returning flattened key-value maps when path is omitted ([#5140](https://github.com/opensearch-project/sql/pull/5140))
* Add `nomv` PPL command to convert multivalue fields into single values delimited by newlines ([#5130](https://github.com/opensearch-project/sql/pull/5130))
...
* Support `LAST`, `FIRST`, and `TAKE` aggregations on TEXT type fields and script expressions ([#5091](https://github.com/opensearch-project/sql/pull/5091))
...
* Add `contains` operator to PPL where clauses as readable alternative to LIKE with wildcards ([#5219](https://github.com/opensearch-project/sql/pull/5219))
* Bump ANTLR version from 4.7.1 to 4.13.2 across all modules ([#5159](https://github.com/opensearch-project/sql/pull/5159))

### Bug Fixes

* Fix `head`/TopK semantics being lost during sort-expression pushdown, returning more rows than requested ([#5135](https://github.com/opensearch-project/sql/pull/5135))
* Return null instead of Infinity for double overflow in arithmetic operations to prevent JSON serialization errors ([#5202](https://github.com/opensearch-project/sql/pull/5202))
...
* Fix path navigation on map columns for `spath` command in non-projection contexts ([#5149](https://github.com/opensearch-project/sql/pull/5149))
...
* Fix typo in ANTLR grammar: rename `renameClasue` to `renameClause` ([#5252](https://github.com/opensearch-project/sql/pull/5252))
* Fix `isnotnull()` not being pushed down when combined with multiple `!=` conditions in Sarg expressions ([#5238](https://github.com/opensearch-project/sql/pull/5238))
...
* Revert dynamic column support for `spath` command in preparation for map-based redesign ([#5139](https://github.com/opensearch-project/sql/pull/5139))
...
* Add release notes for 3.6.0 ([#5309](https://github.com/opensearch-project/sql/pull/5309))
* Add release notes for 3.6.0 ([#5312](https://github.com/opensearch-project/sql/pull/5312))
