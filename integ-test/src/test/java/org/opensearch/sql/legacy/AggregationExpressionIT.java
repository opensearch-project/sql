...
public class AggregationExpressionIT extends SQLIntegTestCase {
  ...
  public void noGroupKeySingleFuncOverAggWithoutAliasShouldPass() {
    JSONObject response =
        executeJdbcRequest(
            String.format(
                Locale.ROOT, "SELECT abs(MAX(age)) " + "FROM %s", Index.ACCOUNT.getName()));
  ...
  public void noGroupKeyMaxAddMinShouldPass() {
    JSONObject response =
        executeJdbcRequest(
            String.format(
                Locale.ROOT,
                "SELECT MAX(age) + MIN(age) as addValue " + "FROM %s",
  ...
  public void noGroupKeyMaxAddLiteralShouldPass() {
    JSONObject response =
        executeJdbcRequest(
            String.format(
                Locale.ROOT, "SELECT MAX(age) + 1 as `add` " + "FROM %s", Index.ACCOUNT.getName()));
  ...
  public void noGroupKeyAvgOnIntegerShouldPass() {
    JSONObject response =
        executeJdbcRequest(
            String.format(
                Locale.ROOT, "SELECT AVG(age) as `avg` " + "FROM %s", Index.BANK.getName()));
  ...
  public void noGroupKeyLogMaxAddMinShouldPass() {
    JSONObject response =
        executeJdbcRequest(
            String.format(
                Locale.ROOT,
                "SELECT Log(MAX(age) + MIN(age)) as `log` " + "FROM %s",
                Index.ACCOUNT.getName()));
  ...
  public void groupByConstantShouldPass() {
    JSONObject response =
        executeJdbcRequest(
            String.format(Locale.ROOT, "select 1 from %s GROUP BY 1", Index.BANK.getName()));
