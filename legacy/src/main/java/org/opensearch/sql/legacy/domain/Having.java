 ...
 *
 * <p>Responsibilities: 1. Parsing: parse conditions out during initialization 2. Explain: translate
 * conditions to OpenSearch query DSL (Bucket Selector Aggregation)
 */
public class Having {
  ...
  /**
   * Explain conditions recursively. Example: HAVING c >= 2 OR NOT (a > 20 AND c <= 10 OR a < 1) OR
   * a < 5 Object: Where(?: [ Condition(?:c>=2), Where(or: [ Where(?:a<=20), Where(or:c>10),
   * Where(and:a>=1)], ]), Condition(or:a<5) ])
   *
   * <p>Note: a) Where(connector : condition expression). b) Condition is a subclass of Where. c)
   * connector=? means it doesn't matter for first condition in the list
   *
