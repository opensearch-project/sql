...
/**
 * Utility class for resolving wildcard patterns in field names for table and fields commands
 * when Calcite engine is not enabled.
 */
public class WildcardFieldResolver {

  /**
   * Resolves wildcard patterns in field expressions to actual field names from the type environment.
   * 
        ...
        if (isWildcardPattern(fieldName)) {
          ...
          for (String matchedField : matchingFields) {
            if (!seenFields.contains(matchedField)) {
              ...
              resolvedFields.add(DSL.named(matchedField, new ReferenceExpression(matchedField, fieldType)));
          ...
          if (!seenFields.contains(fieldName)) {
            org.opensearch.sql.expression.Expression analyzedExpr = expressionAnalyzer.analyze(expr, context);
  ...
  /**
   * Matches a wildcard pattern against available field names.
   * Supports prefix (*name), suffix (account*), and contains (*a*) patterns.
   * 
   ...
   */
  private static List<String> matchWildcardPattern(String pattern, java.util.Set<String> availableFields) {
