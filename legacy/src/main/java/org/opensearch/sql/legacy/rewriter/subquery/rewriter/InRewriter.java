...
/**
 * IN Subquery Rewriter. For example, SELECT * FROM A WHERE a IN (SELECT b FROM B) and c > 10 should
 * be rewritten to SELECT A.* FROM A JOIN B ON A.a = B.b WHERE c > 10 and B.b IS NOT NULL.
 */
public class InRewriter implements Rewriter {
   ...
   *
   * <p>With the input query. Query / | \ SELECT FROM WHERE | | / | \ * A c>10 AND INSubquery / \ a
  ...
  /**
   * Build the Null check expression. For example, SELECT * FROM A WHERE a IN (SELECT b FROM B),
