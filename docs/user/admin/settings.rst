...
They select indices, not rows: an index wholly outside the window is not read. Send them only for a window the query's own ``where`` already restricts -- every source it reads, since a ``join``'s other side and a subsearch are narrowed too -- or the query returns fewer rows than it would without them.
