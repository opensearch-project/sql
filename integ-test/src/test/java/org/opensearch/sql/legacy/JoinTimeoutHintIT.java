 ...
 * Tests the complete flow from SQL parsing through query execution to verify
 * that PIT keepalive is properly set from JOIN_TIME_OUT hint values.
 */
public class JoinTimeoutHintIT extends SQLIntegTestCase {
    ...
    /**
     * Core test: Verify that JOIN_TIME_OUT hint prevents "Point In Time id doesn't exist" errors
     * by setting PIT keepalive to match the hint value instead of using default 60 seconds.
    ...
    public void testJoinTimeoutHintWithDifferentValues() throws IOException {
        ...
        for (int timeout : timeoutValues) {
            String query = String.format(
                    ...
                    timeout, TEST_INDEX_PEOPLE, TEST_INDEX_DOG);
    ...
    public void testJoinTimeoutHintWithMultipleHints() throws IOException {
        String query = String.format(
                Locale.ROOT,
                "SELECT /*! HASH_WITH_TERMS_FILTER*/ /*! JOIN_TIME_OUT(240) */ /*! JOIN_TABLES_LIMIT(500,200) */ " +
                        "a.firstname, d.dog_name " +
                        "FROM %s a JOIN %s d ON d.holdersName = a.firstname " +
                        "WHERE a.age > 25 LIMIT 8",
                TEST_INDEX_PEOPLE, TEST_INDEX_DOG);
    ...
    public void testJoinTimeoutHintConsistency() throws IOException {
        ...
        assertEquals("Results should be consistent between executions",
                resultsCount1, resultsCount2);
        assertEquals("Results should be consistent between executions",
                resultsCount2, resultsCount3);
    }
    ...
    /**
     * Helper method to safely get results count from response
     * Handles both traditional OpenSearch "hits" format and SQL response format
     */
