...
/**
 * Add the alias for identifier the subquery query. Use the table alias if it already has one, Auto
 * generate if it doesn't has one.
 *
 * <p>The following table demonstrate how the rewriter works with scope and query.
 * +-----------------------+-------------+-----------------------------------------------------------------------------------------------------+
 * | Rewrite | TableScope | Query |
 * +-----------------------+-------------+-----------------------------------------------------------------------------------------------------+
 * | (Start) | () | SELECT * FROM TbA WHERE a IN (SELECT b FROM TbB) and c > 10 |
 * +-----------------------+-------------+-----------------------------------------------------------------------------------------------------+
 * | MySqlSelectQueryBlock | (TbA,TbA_0) | SELECT * FROM TbA as TbA_0 WHERE a IN (SELECT b FROM TbB)
 * and c > 10 |
 * +-----------------------+-------------+-----------------------------------------------------------------------------------------------------+
 * | Identifier in Select | (TbA,TbA_0) | SELECT TbA.* FROM TbA as TbA_0 WHERE a IN (SELECT b FROM
 * TbB) and c > 10 |
 * +-----------------------+-------------+-----------------------------------------------------------------------------------------------------+
 * | Identifier in Where | (TbA,TbA_0) | SELECT TbA.* FROM TbA as TbA_0 WHERE TbA_0.a IN (SELECT b
 * FROM TbB) and TbA_0.c > 10 |
 * +-----------------------+-------------+-----------------------------------------------------------------------------------------------------+
 * | MySqlSelectQueryBlock | (TbA,TbA_0) | SELECT TbA.* FROM TbA as TbA_0 WHERE TbA_0.a IN (SELECT b
 * FROM TbB as TbB_0) and TbA_0.c > 10 | | | (TbB,TbB_0) | |
 * +-----------------------+-------------+-----------------------------------------------------------------------------------------------------+
 * | Identifier in Select | (TbA,TbA_0) | SELECT TbA.* FROM TbA as TbA_0 WHERE TbA_0.a IN (SELECT
 * TbB_0.b FROM TbB as TbB_0) and TbA_0.c > 10 | | | (TbB,TbB_0) | |
 * +-----------------------+-------------+-----------------------------------------------------------------------------------------------------+
 */
public class SubqueryAliasRewriter extends MySqlASTVisitorAdapter {
  ...
  private boolean inWhere(SQLIdentifierExpr expr) {
    return expr.getParent() instanceof SQLBinaryOpExpr
        && !isESTable((SQLBinaryOpExpr) expr.getParent());
  }
  ...
  private boolean isESTable(SQLBinaryOpExpr expr) {
    return expr.getOperator() == SQLBinaryOperator.Divide
        && expr.getParent() instanceof SQLExprTableSource;
  }
