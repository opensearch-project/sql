...
public class OpenSearchTypeUtil {
  /**
   * Determines if the given RelDataType is a user-defined type (UDT).
   *
  ...
  /**
   * Determines whether the given RelDataType represents a time-based field (timestamp, date, or time),
   * including standard SQL datetime types and OpenSearch user-defined time types.
   *
   * @param fieldType the RelDataType to check
   * @return `true` if the type is a timestamp, date, or time type, `false` otherwise
   */
