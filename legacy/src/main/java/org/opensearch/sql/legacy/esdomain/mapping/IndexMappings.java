 ...
 *
 * <p>Sample: indexMappings: { 'accounts': typeMappings1, 'logs': typeMappings2 }
 *
 * <p>Difference between response of getMapping/clusterState and getFieldMapping:
 *
 * <p>1) MappingMetadata: ((Map) ((Map)
 * (mapping.get("bank").get("account").sourceAsMap().get("properties"))).get("balance")).get("type")
 *
 * <p>2) FieldMetadata: ((Map)
 * client.admin().indices().getFieldMappings(request).actionGet().mappings().get("bank")
