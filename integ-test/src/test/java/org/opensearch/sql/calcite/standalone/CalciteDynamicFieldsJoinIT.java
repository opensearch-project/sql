...
public class CalciteDynamicFieldsJoinIT extends CalcitePPLPermissiveIntegTestCase {
  ...
  @Test
  public void testJoinStaticWithStaticByFieldName() throws IOException {
    String query =
  ...
  @Test
  public void testJoinStaticWithStaticByCriteria() throws IOException {
    String query =
        source(
            TEST_DYNAMIC_LEFT,
            "join left=l right=r on l.a2 = r.a2 "
                + TEST_DYNAMIC_RIGHT
  ...
  @Test
  public void testJoinDynamicWithStaticByFieldName() throws IOException {
    String query =
    ...
    assertExplainYaml(
        ...
        "calcite:\n"
            ...
            + "  physical: |\n"
            + "    EnumerableCalc(expr#0..5=[{inputs}], expr#6=['a1'], expr#7=[ITEM($t5, $t6)],"
            + " expr#8=[COALESCE($t7, $t0)], expr#9=[MAP_CONCAT($t1, $t5)], expr#10=['a4'],"
            + " expr#11=[ITEM($t9, $t10)], expr#12=['a5'], expr#13=[ITEM($t9, $t12)], a1=[$t8],"
            + " a2=[$t3], a3=[$t4], a4=[$t11], a5=[$t13])\n"
            + "      EnumerableLimit(fetch=[200])\n"
  ...
  @Test
  public void testJoinDynamicWithStaticByCriteria() throws IOException {
    String query =
        source(
            TEST_DYNAMIC_LEFT,
  ...
  @Test
  public void testJoinStaticWithDynamicByFieldName() throws IOException {
    String query =
    ...
    assertExplainYaml(
        ...
        "calcite:\n"
            ...
            + "      LogicalProject(a1=[COALESCE(ITEM($5, 'a1'), $0)], a2=[$3], a3=[$4],"
            + " a4=[ITEM(MAP_CONCAT($2, $5), 'a4')], a5=[ITEM(MAP_CONCAT($2, $5), 'a5')])\n"
            + "        LogicalJoin(condition=[=($0, SAFE_CAST(ITEM($5, 'a1')))],"
            + " joinType=[inner])\n"
            + "          LogicalProject(a1=[$0], a2=[$1], _MAP=[$8])\n"
            + "            CalciteLogicalIndexScan(table=[[OpenSearch, test_dynamic_left]])\n"
            + "          LogicalSystemLimit(fetch=[50000], type=[JOIN_SUBSEARCH_MAXOUT])\n"
  ...
  @Test
  public void testJoinStaticWithDynamicByCriteria() throws IOException {
    String query =
        source(
            TEST_DYNAMIC_LEFT,
            "join left = l right = r on l.a1 = r.a1 "
                + TEST_DYNAMIC_RIGHT
                + " | fields a1, a2, a3, a4, a5");
    JSONObject result = executeQuery(query);
    verifyJoinResult(result);
  ...
  @Test
  public void testJoinDynamicWithDynamicByFieldName() throws IOException {
    String query =
    ...
    assertExplainYaml(
        ...
        "calcite:\n"
            ...
            + " expr#9=[COALESCE($t8, $t0)], expr#10=[MAP_CONCAT($t1, $t5)], expr#11=['a4'],"
  ...
  @Test
  public void testJoinDynamicWithDynamicByCriteria() throws IOException {
    String query =
        source(
            TEST_DYNAMIC_LEFT,
