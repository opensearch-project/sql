 ...
 *
 * <p>Ex. To make concepts clear, for "e.firstname AND t.region" in "FROM team t, t.employees e":
 * parent alias (to erase): 't' path: 'e' (full path saved in Scope is 'employees') name:
 * 'firstname'
 */
class Identifier extends SQLClause<SQLIdentifierExpr> {
  ...
  /**
   * Erase parent alias for all identifiers but only rewrite those (nested field identifier) NOT in
   * WHERE. For identifier in conditions in WHERE, use full path as tag and delay the rewrite in
   * Where.rewrite().
   */
  @Override
  ...
  /**
   * return the path of the expr name. e.g. expecting p returned as path in both WHERE p.name = 'A'
   * and WHERE p IS NULL cases, in which expr.name = p.name and p separately
   */
  String path() {
  ...
  /**
   * Erase parent alias otherwise it's required to specify it everywhere even on nested field (which
   * NLPchina has problem with). Sample: "FROM team t, t.employees e WHERE t.region = 'US'" =>
   * "WHERE region = 'US'"
   */
